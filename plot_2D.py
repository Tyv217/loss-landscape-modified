...
def plot_trajectory_compare(proj_file1, proj_file2, dir_file, name1, name2, losses1, losses2, annotate_every_n1, annotate_every_n2, show=False):
    """Plot optimization trajectory on the plane spanned by given directions."""
    ...
    for i, loss in enumerate(losses1):
        if i % annotate_every_n1 == 0:
            plt.annotate(loss, (x_coord1[i], y_coord1[i] + 0.5))
    for i, loss in enumerate(losses2):
        if i % annotate_every_n2 == 0:
            plt.annotate(loss, (x_coord2[i], y_coord2[i] - 0.5))
    plt.tick_params("y", labelsize="x-large")
    plt.tick_params("x", labelsize="x-large")
    plt.legend()
    f1.close()
